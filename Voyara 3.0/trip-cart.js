(() => {
  ...
  const normalizeTravelers = (item) => {
    if (item.travelers) return { adults: Number(item.travelers.adults || 0) + Number(item.travelers.seniors || 0), kids: Number(item.travelers.kids || 0) };
    return { adults: Number(item.guests || 2), kids: 0 };
  };
  const readTrips = () => {
  ...
  const saveTrip = (trip) => {
    ...
    const guests = travelers.adults + travelers.kids;
  ...
  function render() {
    ...
    itemsRoot.querySelectorAll("[data-quantity]").forEach((button) => button.addEventListener("click", () => {
      ...
      const next = Math.max(key === "adults" ? 1 : 0, counts[key] + Number(button.dataset.quantity));
      counts[key] = next;
